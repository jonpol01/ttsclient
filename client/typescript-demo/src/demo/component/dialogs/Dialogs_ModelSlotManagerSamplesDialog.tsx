...
import { GPTSoVITSSampleInfo, ReservedForSampleSlotInfo, SampleInfo } from "tts-client-typescript-client-lib";
...
type GPTSoVITSDetailAreaProps = {
    sampleInfo: GPTSoVITSSampleInfo;
};
const GPTSoVITSDetailArea = (props: GPTSoVITSDetailAreaProps) => {
    ...
    return (
        <div className={modelSampleDetailArea}>
            ...
            <InfoRow title={t("dialog_sample_tts_type")} text={`${sampleInfo.tts_type}`}></InfoRow>
            <InfoRow
                title={t("dialog_sample_info")}
                text={`ver:${sampleInfo.version}, m_ver:${sampleInfo.model_version}, lora:${sampleInfo.lora_v3}`}
            ></InfoRow>
        </div>
    );
};

type ModelSlotManagerSamplesDialogProps = {
    slotIndex: number;
};

export const ModelSlotManagerSamplesDialog = (props: ModelSlotManagerSamplesDialogProps) => {
    const { t } = useTranslation();
    ...
    const sampleRow = useMemo(() => {
        return serverConfigState.samples.map((x, index) => {
            ...
            if (x.tts_type == "GPT-SoVITS") {
                sampleDetail = <GPTSoVITSDetailArea sampleInfo={x as GPTSoVITSSampleInfo}></GPTSoVITSDetailArea>;
            }
                ...
                <div
                    ...
                    onClick={async () => {
                        ...
                        const checkDownload = async (num: number) => {
                            while (num < 60) {
                                const slotInfo = await serverConfigState.getServerSlotInfo(props.slotIndex)
                                if (slotInfo?.tts_type == null) {
                                    // reserve前。noop
                                } else if (slotInfo?.tts_type == "RESERVED_FOR_SAMPLE") {
                                    const reservedForSampleSlot = slotInfo as ReservedForSampleSlotInfo
                                    setProgress(reservedForSampleSlot.progress);
                                } else {
                                    setProgress(1);
                                    // 終了アクションを待つ
