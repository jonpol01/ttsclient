...
import { Emotion, EmotionBlockButton, EmotionButton, EmotionColors } from "../../styles/style-components/buttons/02_emotion-button.css";
...
export const ReferenceVoiceSelector = () => {
    ...
    const component = useMemo(() => {
        ...
        const voiceCharacterEmotionTypes = voiceCharacter.emotion_types?.map(x => { return x.name }) || []
        ...
        const generateEmotionColorMap = () => {
            ...
            const referenceVoiceEmotionTypes = voiceCharacter.reference_voices.map(x => { return x.voice_type })
            const referenceVoiceEmotionTypesSet = new Set(referenceVoiceEmotionTypes)
            const unregisteredEmotionTypes = Array.from(referenceVoiceEmotionTypesSet).filter(x => { return !voiceCharacterEmotionTypes.includes(x) })
            const usedColors = voiceCharacter.emotion_types.map(x => { return x.color })
        ...
        const voiceBlocks = Array.from({ length: row }, (_, i) => {
            return Array.from({ length: col }, (_, j) => {
                ...
                return (
                    ...
                    <button key={cellIndex} className={EmotionBlockButton({ selected: selected })}
        ...
        const sideButtons = Array.from(emotionColorMap.keys()).map((x, i) => {
            return (
                <button key={x} className={EmotionButton()}
